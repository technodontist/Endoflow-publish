...
export async function getActivePatients(clinicId?: string): Promise<Patient[]> {
  ...
  try {
    ...
    // If clinicId provided, only return patients belonging to this clinic
    if (clinicId) {
...
export async function getPendingAppointmentRequests(dentistId?: string): Promise<AppointmentRequest[]> {
  ...
  try {
    ...
    // Filter by preferred dentist if provided
    if (dentistId) {
      query = query.eq('preferred_dentist_id', dentistId);
    }
