...
import { ConsultationImageUploader, type UploadedImage } from '@/components/consultation/ConsultationImageUploader'
...
import { generateConsultationReportAction } from '@/lib/actions/consultation-report'
...
interface ConsultationData {
  ...
  // Session 20: Voice-extracted structured data objects (populated by process-global-transcript)
  // These hold the full structured extraction from each section for tab consumption.
  chiefComplaintData?: any
  hopiData?: any
  medicalHistoryData?: any
  personalHistoryData?: any
  clinicalExaminationData?: any
...
interface EnhancedNewConsultationProps {
  ...
  /** Session 12: Callback when consultation is saved — used by V5 to show Generate Report button */
  onConsultationSaved?: (consultationId: string, patientId: string) => void
  /** Session 18: Mobile-optimized layout — collapsible chart, stacked uploads, larger touch targets */
...
export function EnhancedNewConsultationV4({ selectedPatientId, appointmentId, dentistId, onPatientSelect, onConsultationSaved, isMobileView = false }: EnhancedNewConsultationProps) {
  ...
  // Session 12: Consultation image upload state
  const [consultationImages, setConsultationImages] = useState<any[]>([])

  // Session 17: Voice-first UI state
  const [liveTranscript, setLiveTranscript] = useState('')
  const [recordingState, setRecordingState] = useState<{
    isRecording: boolean; isPaused: boolean; isProcessing: boolean; duration: number; segmentCount: number
  }>({ isRecording: false, isPaused: false, isProcessing: false, duration: 0, segmentCount: 0 })
  const [isCaseHistoryExpanded, setIsCaseHistoryExpanded] = useState(false)
  // Session 18: FDI chart collapsed by default on mobile to save vertical space
  const [isMobileChartVisible, setIsMobileChartVisible] = useState(false)
  const [isGeneratingPdf, setIsGeneratingPdf] = useState(false)

  const autoSaveRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  
  // NEW: Ref to track voice-extracted teeth that shouldn't be overwritten by DB reloads
  const voiceExtractedTeethRef = useRef<Set<string>>(new Set())
  
  // NEW: Ref to track initial teeth when patient selected (for real-time display filtering)
  ...
  // Session 18b: Build ConversationContext from consultationData for the AI pipeline
  // Maps flat consultationData fields to the nested ConversationContext interface
  // used by DiagnosticGapDialog → runPipelineAction → all AI agents
  const buildConversationContext = useCallback(() => {
    const ctx: Record<string, any> = {}

    // Chief Complaint
    if (consultationData.chiefComplaint || consultationData.painLocation) {
      ctx.chiefComplaint = {
        primary_complaint: consultationData.chiefComplaint || '',
        patient_description: consultationData.painLocation || '',
        onset_duration: consultationData.painDuration || '',
        associated_symptoms: [],
        triggers: consultationData.painTriggers?.length ? consultationData.painTriggers : [],
      }
    }

    // HOPI (History of Present Illness)
    if (consultationData.painCharacter || consultationData.painIntensity || consultationData.hopiOnsetDetails) {
      ctx.hopi = {
        ...
        aggravating_factors: consultationData.painTriggers?.length ? consultationData.painTriggers : [],
        relieving_factors: consultationData.painRelief?.length ? consultationData.painRelief : [],
        associated_symptoms: [],
        previous_treatments: consultationData.previousDentalTreatments?.length ? consultationData.previousDentalTreatments : [],
      }
    }

    // Medical History
    if (consultationData.medicalHistory?.length || consultationData.currentMedications?.length || consultationData.allergies?.length) {
      ctx.medicalHistory = {
        medical_conditions: consultationData.medicalHistory?.length ? consultationData.medicalHistory : [],
        current_medications: consultationData.currentMedications?.length ? consultationData.currentMedications : [],
        allergies: consultationData.allergies?.length ? consultationData.allergies : [],
        previous_dental_treatments: consultationData.previousDentalTreatments?.length ? consultationData.previousDentalTreatments : [],
      }
    }

    // Clinical Examination
    if (consultationData.extraoralFindings || consultationData.intraoralFindings || consultationData.oralHygieneStatus) {
      ctx.clinicalExamination = {
        extraoral_findings: consultationData.extraoralFindings ? [consultationData.extraoralFindings] : [],
        intraoral_findings: consultationData.intraoralFindings ? [consultationData.intraoralFindings] : [],
        oral_hygiene: consultationData.oralHygieneStatus || '',
        gingival_condition: consultationData.gingivalCondition || '',
      }
    }

    // Session 20: Investigations & Clinical Tests — CRITICAL for gap analyzer accuracy
    // Without these, the gap analyzer asks redundant questions about tests already performed.
    const investigationParts: string[] = []
    if (consultationData.vitalityTests) {
      investigationParts.push(`Vitality tests: ${consultationData.vitalityTests}`)
    }
    if (consultationData.percussionTests) {
      investigationParts.push(`Percussion test: ${consultationData.percussionTests}`)
    }
    if (consultationData.radiographicFindings) {
      investigationParts.push(`Radiographic findings: ${consultationData.radiographicFindings}`)
    }
    if (consultationData.radiographicTypes?.length) {
      investigationParts.push(`Radiographic types: ${consultationData.radiographicTypes.join(', ')}`)
    }
    if ((consultationData as any).palpationTests) {
      investigationParts.push(`Palpation test: ${(consultationData as any).palpationTests}`)
    }
    if (consultationData.investigationRecommendations) {
      investigationParts.push(`Recommendations: ${consultationData.investigationRecommendations}`)
    }
    // Merge investigation findings into clinical examination intraoral_findings
    // so the checklist matcher sees them alongside other clinical data
    if (investigationParts.length > 0) {
      if (!ctx.clinicalExamination) {
        ctx.clinicalExamination = { extraoral_findings: [], intraoral_findings: [], oral_hygiene: '', gingival_condition: '' }
      }
      ctx.clinicalExamination.intraoral_findings = [
        ...(ctx.clinicalExamination.intraoral_findings || []),
        ...investigationParts,
      ]
    }

    // Session 20: Also set explicit investigations field on context
    if (investigationParts.length > 0) {
      ctx.investigations = {
        vitality_tests: consultationData.vitalityTests || undefined,
        percussion_test: consultationData.percussionTests || undefined,
        palpation_test: (consultationData as any).palpationTests || undefined,
        radiographic_findings: consultationData.radiographicFindings || undefined,
        radiographic_types: consultationData.radiographicTypes?.length ? consultationData.radiographicTypes : undefined,
      }
    }

    // Session 20: Include diagnosis and treatment data if already set (from voice extraction or manual input)
    if (consultationData.diagnosisData || (consultationData as any).diagnosis) {
      const diagData = consultationData.diagnosisData || {}
      if (diagData.provisional_diagnosis || (consultationData as any).provisionalDiagnosis) {
        if (!ctx.clinicalExamination) {
          ctx.clinicalExamination = { extraoral_findings: [], intraoral_findings: [], oral_hygiene: '', gingival_condition: '' }
        }
        ctx.clinicalExamination.intraoral_findings = [
          ...(ctx.clinicalExamination.intraoral_findings || []),
          `Provisional diagnosis: ${diagData.provisional_diagnosis || (consultationData as any).provisionalDiagnosis}`,
        ]
      }
    }
  ...
  // FIXED: Merge with voice-extracted teeth instead of replacing entirely
  useEffect(() => {
    const run = async () => {
      if (!selectedPatient?.id) return
      
      console.log('🔄 [REALTIME-EFFECT] Triggered by historyVersion:', historyVersion)
      console.log('🦷 [REALTIME-EFFECT] Voice-extracted teeth to preserve:', Array.from(voiceExtractedTeethRef.current))
      ...
      try {
        ...
        if (error) {
          console.warn('⚠️ [REALTIME-EFFECT] Database query error:', error)
          return
        }
        ...
        for (const row of (data || [])) {
          ...
          latestByTooth[t] = {
            status: (row as any).status || 'healthy',  // FIXED: Add status field for InteractiveDentalChart
        ...
        console.log('💾 [REALTIME-EFFECT] Loaded', Object.keys(latestByTooth).length, 'teeth from database')
        
        // CRITICAL FIX: Merge with current toothData to preserve voice-extracted teeth
        setToothData(prev => {
          console.log('🔀 [REALTIME-EFFECT] Merging DB data with existing toothData')
          console.log('  📊 Previous toothData count:', Object.keys(prev).length)
          console.log('  📊 DB loaded teeth count:', Object.keys(latestByTooth).length)
          
          // Start with database data
          const merged = { ...latestByTooth }
          
          // Preserve voice-extracted teeth that aren't in the database yet
          voiceExtractedTeethRef.current.forEach(toothNum => {
            if (prev[toothNum] && !merged[toothNum]) {
              console.log(`  ✅ [REALTIME-EFFECT] Preserving voice-extracted tooth #${toothNum}`)
              merged[toothNum] = prev[toothNum]
            } else if (prev[toothNum] && merged[toothNum]) {
              console.log(`  ⚠️ [REALTIME-EFFECT] Tooth #${toothNum} now in DB, using DB version`)
              // If voice-extracted tooth is now in DB, remove from tracking
              voiceExtractedTeethRef.current.delete(toothNum)
            }
          })
          
          console.log('  📊 Final merged toothData count:', Object.keys(merged).length)
          console.log('  🦷 Remaining voice-extracted teeth:', Array.from(voiceExtractedTeethRef.current))
          
          return merged
        })
      } catch (err) {
        console.error('❌ [REALTIME-EFFECT] Exception:', err)
      }
    }
    run()
  ...
  // Session 17: Listen for copilot sidebar "Run AI Diagnosis Pipeline" button
  useEffect(() => {
    const handleRunPipeline = () => {
      if (selectedTooth) {
        console.log('🧬 [COPILOT] Opening diagnosis pipeline for tooth', selectedTooth)
        setShowToothInterface(true)
      } else {
        // No tooth selected — try to find the first tooth mentioned in transcript
        const toothMatch = liveTranscript.match(/tooth\s*(?:number|#)?\s*(\d{1,2})/i)
        if (toothMatch) {
          const toothNum = toothMatch[1]
          console.log('🧬 [COPILOT] Auto-selecting tooth', toothNum, 'from transcript')
          setSelectedTooth(toothNum)
          setShowToothInterface(true)
        } else {
          console.log('🧬 [COPILOT] No tooth selected — prompting user')
          alert('Please select a tooth from the FDI chart first, or mention a tooth number in the transcript.')
        }
      }
    }
    window.addEventListener('endoflow:run_diagnosis_pipeline', handleRunPipeline)
    return () => window.removeEventListener('endoflow:run_diagnosis_pipeline', handleRunPipeline)
  }, [selectedTooth, liveTranscript])

  // Session 18: Listen for copilot "Next Tooth" — reset selection, scroll to FDI chart
  useEffect(() => {
    const handleCopilotSelectTooth = () => {
      console.log('🦷 [COPILOT] Next tooth requested — resetting selection')
      setSelectedTooth(null)
      setShowToothInterface(false)
      // Scroll FDI chart into view
      const chartEl = document.querySelector('[data-fdi-chart]')
      if (chartEl) chartEl.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }
    window.addEventListener('endoflow:copilot_select_tooth', handleCopilotSelectTooth)
    return () => window.removeEventListener('endoflow:copilot_select_tooth', handleCopilotSelectTooth)
  }, [])

  // Session 18: Listen for tooth_diagnosis_saved from copilot Accept — reload chart data
  useEffect(() => {
    const handleToothSaved = async (e: Event) => {
      const detail = (e as CustomEvent).detail
      if (detail?.patientId && detail.patientId === selectedPatient?.id) {
        console.log('✅ [COPILOT] Tooth diagnosis saved externally, reloading chart data')
        await reloadToothDiagnoses()
      }
    }
    window.addEventListener('endoflow:tooth_diagnosis_saved', handleToothSaved)
    return () => window.removeEventListener('endoflow:tooth_diagnosis_saved', handleToothSaved)
    ...
    try {
      ...
      if ((result as any).data) {
        ...
        if (r.data.toothData && Object.keys(r.data.toothData).length > 0) {
          ...
          setToothData(prev => {
            const merged = { ...prev, ...r.data.toothData }
  ...
  const handlePatientSelect = async (patient: Patient) => {
    ...
    // CRITICAL FIX: Load patient's existing tooth diagnoses FIRST before anything else
    console.log('🦷 [PATIENT-SELECT] Step 1: Loading all existing tooth diagnoses for patient')
    try {
      ...
      if (toothResult.success && toothResult.data) {
        ...
        const formattedToothData: {[key: string]: any} = {}
        Object.entries(teeth).forEach(([toothNumber, diagnosis]: [string, any]) => {
        ...
        console.log('🦷 [PATIENT-SELECT] Existing teeth loaded:', Object.keys(formattedToothData).sort((a, b) => parseInt(a) - parseInt(b)).join(', '))
        
        // Track initial teeth for session-based filtering in real-time display
        ...
        console.log('ℹ️ [PATIENT-SELECT] No existing tooth diagnoses found for patient')
        setToothData({}) // Start fresh if no existing teeth
        initialTeethRef.current.clear()
        sessionModifiedTeethRef.current.clear()
      }
    } catch (error) {
      console.error('❌ [PATIENT-SELECT] Error loading existing tooth diagnoses:', error)
      setToothData({}) // Fallback to empty on error
      initialTeethRef.current.clear()
      sessionModifiedTeethRef.current.clear()
    }
    
    // Load previous consultation data (this might update toothData if there's draft data)
    console.log('🦷 [PATIENT-SELECT] Step 2: Loading previous consultation data (if any)')
    ...
    if (!savedConsultationId) {
      console.log('🎤 [PATIENT-SELECT] Step 3: Creating draft consultation for voice recording...')
      try {
        const result = await saveConsultationSectionAction({
    ...
    console.log('✅ [PATIENT-SELECT] Patient selection complete')
  }
  ...
  const handleSaveConsultation = async (status: 'draft' | 'completed' = 'draft') => {
    ...
    try {
      ...
      // Session 12: Collect uploaded image file IDs
      const imageFileIds = consultationImages
        .filter(img => img.status === 'done' && img.id && !img.id.startsWith('temp-'))
        .map(img => img.id)
      ...
      const result = await finalizeConsultationFromDraftAction({
        ...
        imageReferences: imageFileIds.length > 0 ? imageFileIds : undefined,
      })
      ...
      if (newId) {
        ...
        // Session 12: Notify parent (V5) so Generate Report button becomes visible
        if (selectedPatient?.id) {
          onConsultationSaved?.(newId, selectedPatient.id)
        }
      }
  ...
  if (!selectedPatient) {
    return (
      <div className="space-y-4 md:space-y-6">
        <div>
          <h1 className={`${isMobileView ? 'text-xl' : 'text-2xl'} font-bold text-foreground`}>New Consultation</h1>
          <p className="text-sm text-muted-foreground">Search and select a patient to begin</p>
        </div>
        <Card>
          <CardContent className={isMobileView ? 'p-3' : 'p-6'}>
            <div className="relative">
              ...
              <Input
                ...
                className={`pl-12 ${isMobileView ? 'text-base h-11' : 'text-lg h-12'}`}
              />
            </div>
            {patients.length > 0 && (
  ...
  // Session 17: Generate PDF report
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      if (result.success && result.pdfBase64) {
        ...
        link.download = result.fileName || `consultation-${savedConsultationId.slice(0, 8)}.pdf`
        link.click()
      } else {
  ...
  // Session 17: Helper to format recording duration
    ...
    <div className={isMobileView ? 'space-y-3' : 'space-y-4'}>
      ...
      <div className={`flex ${isMobileView ? 'flex-col gap-2' : 'items-center justify-between'}`}>
        <div className="flex items-center gap-2 md:gap-3">
          <Button variant="ghost" size="sm" onClick={() => setSelectedPatient(null)} className="shrink-0">
            <ArrowLeft className="w-4 h-4" />
          </Button>
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className={`${isMobileView ? 'text-base' : 'text-lg'} font-bold text-foreground truncate`}>
                {selectedPatient.first_name} {selectedPatient.last_name}
              </h1>
              <Badge variant="outline" className="text-xs">
                UH{selectedPatient.id.slice(-6)}
              </Badge>
              <Badge variant="outline" className="text-xs text-muted-foreground">
                {(() => { const age = getPatientAge(selectedPatient.date_of_birth); return age !== null ? `${age}y` : '—' })()}
              </Badge>
          ...
          {recordingState.isRecording && (
            <div className="flex items-center gap-2 ml-4">
              ...
              {recordingState.segmentCount > 0 && (
                <span className="text-xs text-muted-foreground">{recordingState.segmentCount + 1} seg</span>
              )}
            </div>
          )}
        </div>
        <div className={`flex gap-2 ${isMobileView ? 'flex-wrap' : ''}`}>
          <Button
            variant="outline"
            size="sm"
          ...
          <Button
            size="sm"
            className={`bg-teal-600 hover:bg-teal-700 ${isMobileView ? 'text-xs px-2' : ''}`}
            onClick={() => handleSaveConsultation('completed')}
            disabled={isSaving}
          >
            <Send className="w-4 h-4 mr-1 md:mr-2" />
            {isSaving ? 'Completing...' : isMobileView ? 'Complete' : 'Complete Consultation'}
          </Button>
          {/* Session 17: PDF Export button — visible after any save */}
          {savedConsultationId && (
            <Button
              variant="outline"
              size="sm"
              ...
              className={`border-blue-500 text-blue-400 hover:bg-blue-500/10 ${isMobileView ? 'text-xs px-2' : ''}`}
            >
              <FileText className="w-4 h-4 mr-1 md:mr-2" />
              {isGeneratingPdf ? 'Generating...' : isMobileView ? 'PDF' : 'Export PDF'}
            </Button>
          )}
      ...
      {/* ═══ SECTION 1: LIVE TRANSCRIPT (PRIMARY FOCUS) ═══ */}
      <Card className={`border ${recordingState.isRecording ? 'border-red-500/40 bg-card' : 'border-border bg-card'}`}>
        <CardContent className={isMobileView ? 'p-3' : 'p-4'}>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Live Transcript</span>
          ...
          <div className="min-h-[120px] max-h-[300px] overflow-y-auto rounded-lg bg-muted/50 border border-border p-3">
            {liveTranscript ? (
              <p className="text-sm leading-relaxed text-foreground whitespace-pre-wrap">{liveTranscript}</p>
            ) : (
              <div className="flex items-center justify-center h-[100px] text-muted-foreground">
                <div className="text-center">
                  <Mic className="w-8 h-8 mx-auto mb-2 opacity-30" />
                  <p className="text-sm">Voice transcript will appear here when recording starts</p>
                  <p className="text-xs mt-1 opacity-60">Say &quot;Start consultation&quot; or use the sidebar controls</p>
                </div>
              </div>
            )}
          </div>
          ...
          {liveTranscript && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {(() => {
                const sections = getConsultationSections()
                const filled = sections.filter(s => s.status === 'complete').length
                const partial = sections.filter(s => s.status === 'partial').length
                return (
                  <>
                    {filled > 0 && (
                      <Badge variant="outline" className="text-xs bg-green-500/10 text-green-400 border-green-500/30">
                        <CheckCircle className="w-3 h-3 mr-1" /> {filled} sections filled
                      </Badge>
                    )}
                    {partial > 0 && (
                      <Badge variant="outline" className="text-xs bg-yellow-500/10 text-yellow-400 border-yellow-500/30">
                        <Clock className="w-3 h-3 mr-1" /> {partial} partial
                      </Badge>
                    )}
                  </>
                )
      ...
      {/* ═══ SECTION 2: COMPACT FDI DENTAL CHART ═══ */}
      <Card className="border-border" data-fdi-chart>
        <CardContent className={isMobileView ? 'p-3' : 'p-4'}>
          <div
            className={`flex items-center justify-between ${isMobileView && !isMobileChartVisible ? 'cursor-pointer' : 'mb-3'}`}
            onClick={isMobileView ? () => setIsMobileChartVisible(!isMobileChartVisible) : undefined}
          >
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">FDI Dental Chart</span>
              {selectedTooth && (
                <Badge variant="secondary" className="text-xs bg-blue-500/15 text-blue-400">
                  #{selectedTooth}
                </Badge>
              )}
              {/* Session 18: Show/Hide toggle on mobile */}
              {isMobileView && (
                <span className="text-xs text-muted-foreground">
                  {isMobileChartVisible ? '(tap to hide)' : '(tap to show)'}
                </span>
              )}
            </div>
            {/* Legend — hidden on mobile when chart collapsed */}
            {(!isMobileView || isMobileChartVisible) && (
              <div className={`flex items-center gap-2 text-xs text-muted-foreground ${isMobileView ? 'flex-wrap gap-1' : ''}`}>
                <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 rounded-sm bg-red-500/30 border border-red-500/50" /><span>Caries</span></div>
                <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 rounded-sm bg-purple-500/30 border border-purple-500/50" /><span>RCT</span></div>
                <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 rounded-sm bg-blue-500/30 border border-blue-500/50" /><span>Filled</span></div>
                <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 rounded-sm bg-yellow-500/30 border border-yellow-500/50" /><span>Attention</span></div>
              </div>
            )}
            {isMobileView && (
          ...
          {/* Chart — always visible on desktop, toggleable on mobile */}
          {(!isMobileView || isMobileChartVisible) && (
            <div className={isMobileView ? 'mt-3 overflow-x-auto' : ''}>
              <InteractiveDentalChart
                ...
                onToothSelect={(toothNumber) => {
                  ...
                  setShowToothInterface(true)
                  // Session 18: Notify copilot sidebar of tooth selection
                  window.dispatchEvent(new CustomEvent('endoflow:tooth_selected', {
                    detail: { toothNumber, patientId: selectedPatient?.id }
                  }))
                }}
                ...
                onToothStatusChange={async (toothNumber, status, data) => {
                  ...
                  setTimeout(async () => { await reloadToothDiagnoses() }, 1000)
                }}
              />
            </div>
          )}
      ...
      {/* ═══ SECTION 3: UPLOAD BLOCKS ═══ */}
      {selectedPatient && (
        <ConsultationImageUploader
          patientId={selectedPatient.id}
          consultationId={savedConsultationId || undefined}
          images={consultationImages}
          onImagesChange={setConsultationImages}
          maxImages={10}
        />
      )}

      {/* ═══ SECTION 4: COLLAPSED CASE HISTORY ═══ */}
      <Card className="border-border">
        <div
          className="flex items-center justify-between p-4 cursor-pointer hover:bg-muted/50 transition-colors"
          onClick={() => setIsCaseHistoryExpanded(!isCaseHistoryExpanded)}
        >
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {(() => {
                  ...
                  const partial = sections.filter(s => s.status === 'partial').length
                  return `${completed} complete, ${partial} in progress, ${sections.length - completed - partial} remaining`
                })()}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 md:gap-3">
            {/* Progress pills — hidden on mobile, shown on desktop */}
            <div className="hidden md:flex gap-0.5">
              {getConsultationSections().map((section) => (
                <div
                  key={section.id}
                  className={`w-4 h-1 rounded-full ${
                    section.status === 'complete' ? 'bg-teal-500' :
          ...
          <CardContent className="pt-0 pb-4 px-4">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {getConsultationSections().map((section) => {
                ...
                const hasData = section.status !== 'empty'
                return (
                  <div
                    key={section.id}
                    className={`p-3 rounded-lg border cursor-pointer transition-all hover:bg-muted/50 ${
                      hasData ? 'border-l-2 border-l-teal-500 border-border' : 'border-border'
                    }`}
                  ...
                  >
                    ...
                    <span className={`text-xs ${
                      ...
                      section.status === 'partial' ? 'text-yellow-400' :
                      'text-muted-foreground'
                    }`}>
                      {section.status === 'empty' ? 'Not started' : section.status === 'partial' ? 'In progress' : 'Complete'}
                    </span>
                  </div>
                )
              })}
      ...
      {/* ═══ GLOBAL VOICE RECORDER (headless — logic only, UI in sidebar) ═══ */}
      <GlobalVoiceRecorder
        consultationId={savedConsultationId || ''}
        ...
        onToothDiagnosesExtracted={(toothDiagnoses) => {
          ...
          toothDiagnoses.forEach(diag => {
            ...
            sessionModifiedTeethRef.current.add(diag.toothNumber)
          })
          setPendingVoiceToothDiagnoses(prev => {
            ...
            toothDiagnoses.forEach(newDiag => {
              ...
              if (existingIndex >= 0) { merged[existingIndex] = newDiag } else { merged.push(newDiag) }
            })
            return merged
          })
          setToothData(prev => {
      ...
      <ToothDiagnosisDialogV2
        ...
        rawTranscript={liveTranscript || undefined}
        patientAge={selectedPatient?.date_of_birth ? differenceInYears(new Date(), new Date(selectedPatient.date_of_birth)) : null}
        existingData={(() => {
