    ...
    <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-8 gap-1">
                  {[18, 17, 16, 15, 14, 13, 12, 11, 21, 22, 23, 24, 25, 26, 27, 28].map(toothNum => {
                    ...
                    const isNewOrModified = !initialTeethRef.current.has(toothNum.toString()) || sessionModifiedTeethRef.current.has(toothNum.toString())
      ...
      <GlobalVoiceRecorder
        ...
        onToothDiagnosesExtracted={(toothDiagnoses) => {
          ...
          // Track voice-extracted teeth to prevent them from being overwritten by DB reloads
          toothDiagnoses.forEach(diag => {
            voiceExtractedTeethRef.current.add(diag.toothNumber)
            sessionModifiedTeethRef.current.add(diag.toothNumber)  // Also mark as session-modified
            console.log(`🎙️ [VOICE] Tracking tooth #${diag.toothNumber} as voice-extracted and session-modified`)
          })
          console.log('🎙️ [VOICE] Total voice-extracted teeth being tracked:', Array.from(voiceExtractedTeethRef.current))
          ...
          setToothData(prev => {
            console.log('🔀 [VOICE] Merging voice diagnoses into toothData')
            console.log('  📊 [VOICE] Current toothData count:', Object.keys(prev).length)
            console.log('  📊 [VOICE] Incoming voice diagnoses:', toothDiagnoses.length)
            
            const updated = { ...prev }  // PRESERVE all existing teeth
            toothDiagnoses.forEach(diag => {
              console.log(`  🦷 [VOICE] Adding/updating tooth #${diag.toothNumber} with status='${diag.status}'`)
            ...
            console.log('✅ [VOICE] Updated toothData with', toothDiagnoses.length, 'voice diagnoses')
            console.log('  📊 [VOICE] Final toothData count:', Object.keys(updated).length)
            console.log('  🦷 [VOICE] All teeth in toothData:', Object.keys(updated).sort((a, b) => parseInt(a) - parseInt(b)).join(', '))
